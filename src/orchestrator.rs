...
pub fn build_dependency_graph(deps: &[TaskDependency]) -> Vec<Vec<String>> {
    ...
    if order.len() != in_degree.len() {
        panic!(
            "circular dependency detected — {} tasks remain unvisited",
...
pub fn save_learning(task_desc: &str, result: &SubTaskResult) -> std::io::Result<()> {
    ...
    if std::path::Path::new(&lp).exists() {
        ...
        let h =
            "| Timestamp | Status | Task | Duration | Verify |\n| --- | --- | --- | --- | --- |\n";
...
fn detect_modules(description: &str) -> Vec<String> {
    ...
    for &module in KNOWN_MODULES {
        ...
        let pat = format!(" {ml} ");
        if (lower.contains(&pat) || lower.starts_with(&ml))
...
pub fn plan_parallel_groups(tasks: &[SubTask]) -> Vec<Vec<usize>> {
    ...
    for task_idx in 0..n {
        ...
        for group in groups.iter_mut() {
            let conflicts = group
                .iter()
                .any(|&oi| !task_files.is_disjoint(&file_sets[oi]));
            if !conflicts {
                group.push(task_idx);
                placed = true;
                break;
...
fn run_subtask(task: &SubTask) -> SubTaskResult {
    ...
    let ctx = context::compress_for_worker(
        &task.prompt,
        &project_md,
        &vault_entries,
        &error_matches,
...
pub fn plan_and_run(description: &str, max_parallel: usize) -> OrchestrationResult {
    ...
    if total_cost > 0.0 {
        println!(
            "💰 Estimated total: ~${:.4} (~{} tokens)",
            total_cost / 100.0,
            tasks.iter().map(|t| t.prompt.len() / 4).sum::<usize>()
        );
    }
    ...
    for r in &result.results {
        if save_learning(&r.description, r).is_ok() {
            ls += 1;
        }
    }
    if ls > 0 {
        println!("🧠 Saved {ls} learning entries to vault/learning/");
    }
    result
}
...
mod tests {
    ...
    fn test_dep_chain() {
        ...
        assert_eq!(lvls[2], vec!["C"]);
    }
    #[test]
    fn test_dep_siblings() {
    ...
    #[test]
    fn test_dep_empty() {
        assert!(build_dependency_graph(&[]).is_empty());
    }
    #[test]
    #[should_panic(expected = "circular")]
    fn test_dep_cycle() {
        build_dependency_graph(&[TD("A", &["B"]), TD("B", &["A"])]);
    }
    #[test]
    fn test_format_tree() {
    ...
    fn test_format_tree_empty() {
        assert_eq!(format_dependency_tree(&[]), "(empty)");
    }

    #[test]
    fn test_auto_verify_clean() {
        let task = SubTask {
            id: 0,
            description: "t".into(),
            files: vec!["src/orchestrator.rs".into()],
            prompt: "t".into(),
            can_parallel: false,
        };
        let (p, o) = auto_verify(&task);
        assert!(p, "verify should pass: {o}");
    }
    #[test]
    fn test_auto_verify_nonexistent() {
        let task = SubTask {
            id: 0,
            description: "t".into(),
            files: vec!["src/nope_xyz.rs".into()],
            prompt: "t".into(),
            can_parallel: false,
        };
        let (_, o) = auto_verify(&task);
        assert!(!o.is_empty());
    }
    ...
    fn test_cost_openai() {
        ...
        assert_eq!(e.prompt_chars, 13);
        assert_eq!(e.estimated_tokens, 3);
    }
    #[test]
    fn test_cost_anthropic() {
        let e = estimate_cost("A".repeat(4000).as_str(), "anthropic");
        assert_eq!(e.estimated_tokens, 1000);
    }
    #[test]
    fn test_cost_ollama() {
        assert_eq!(estimate_cost("p", "ollama").estimated_cost_cents, 0.0);
    }
    #[test]
    fn test_cost_unknown() {
        assert_eq!(estimate_cost("t", "unk").price_per_1k_tokens_cents, 0.5);
    }
    #[test]
    fn test_cost_empty() {
        let e = estimate_cost("", "openai");
        assert_eq!(e.prompt_chars, 0);
    }
    #[test]
    fn test_price_per_1k() {
    ...
    fn test_save_learning() {
        let r = SubTaskResult {
            ...
            duration_secs: 1.5,
            verify_passed: true,
            verify_output: "pass".into(),
        };
        assert!(save_learning("Test", &r).is_ok());
    }
    #[test]
    fn test_save_learning_fail() {
        let r = SubTaskResult {
            id: 1,
            description: "tf".into(),
            success: false,
    ...
    #[test]
    fn test_bar_empty() {
        assert_eq!(render_progress_bar(0, 10, 10), "[░░░░░░░░░░] 0/10");
    }
    #[test]
    fn test_bar_full() {
        assert_eq!(render_progress_bar(10, 10, 10), "[██████████] 10/10");
    }
    #[test]
    fn test_bar_half() {
        assert_eq!(render_progress_bar(5, 10, 10), "[█████░░░░░] 5/10");
    }
    #[test]
    fn test_bar_zero() {
        assert_eq!(render_progress_bar(0, 0, 10), "[░░░░░░░░░░] 0/0");
    }
    #[test]
    fn test_bar_width() {
        assert_eq!(render_progress_bar(3, 6, 6), "[███░░░] 3/6");
    }
    #[test]
    fn test_bar_round() {
        assert_eq!(render_progress_bar(1, 3, 10), "[███░░░░░░░] 1/3");
    }

    #[test]
    fn test_split_modules() {
        let t = split_tasks("add tests to parallel.rs and dashboard.rs");
        assert_eq!(t.len(), 2);
    }
    #[test]
    fn test_split_single() {
        let t = split_tasks("fix the parallel module");
        assert!(!t.is_empty());
    }
    #[test]
    fn test_groups_no_conflict() {
        let t = vec![st(0, "a", true), st(1, "b", true), st(2, "c", true)];
        assert_eq!(plan_parallel_groups(&t).len(), 1);
    }
    #[test]
    fn test_groups_conflict() {
        let t = vec![st_file(0, "shared"), st_file(1, "shared")];
        assert!(plan_parallel_groups(&t).len() >= 2);
    }
    #[test]
    fn test_groups_mixed() {
        let t = vec![
            st_file(0, "a"),
            st_file(1, "b"),
            st_file(2, "a"),
    ...
    fn st(id: usize, file: &str, cp: bool) -> SubTask {
        SubTask {
            ...
            files: vec![format!("src/{file}.rs")],
            prompt: "".into(),
            can_parallel: cp,
        }
    }
    fn st_file(id: usize, file: &str) -> SubTask {
        st(id, file, false)
    }
    fn st_file2(id: usize, f1: &str, f2: &str) -> SubTask {
        SubTask {
            id,
            description: format!("{f1}+{f2}"),
            files: vec![format!("src/{f1}.rs"), format!("src/{f2}.rs")],
            prompt: "".into(),
            can_parallel: true,
        }
    }
